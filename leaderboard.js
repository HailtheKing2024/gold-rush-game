...
function formatScore(entry) {
    ...
    return `<tr><td>${entry.name}</td><td>${entry.score}</td><td>${date.toLocaleString()}</td></tr>`;
}
...
if (document.getElementById('scores-container')) {
    ...
    async function refresh() {
        try {
            ...
            container.innerHTML = `<table><thead><tr><th>Name</th><th>Score</th><th>Date</th></tr></thead><tbody>${rows}</tbody></table>`;
        } catch (err) {
